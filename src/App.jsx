...
export default function App() {
  ...
  // LISTA INTELIGENTE DE PROVEEDORES
  ...
  // --- CEREBRO DEL RESUMEN GERENCIAL (SOLO ADMIN) ---
  useEffect(() => {
    const calcularResumen = () => {
        // 1. Filtramos TODO lo que pasó en la fecha seleccionada
        const movsFecha = listaCompras.filter(m => m.fecha === fecha);

        // 2. Separamos por sede
        const movsRiv = movsFecha.filter(m => m.sede && m.sede.includes("Rivera"));
        const movsBar = movsFecha.filter(m => m.sede && m.sede.includes("Barcelona"));
    ...
    calcularResumen();
  }, [fecha, listaCompras]); // Se recalcula si cambias fecha o alguien registra algo
  ...
  // VISTA ADMIN (KAMALA) - AQUÍ ESTÁ EL RESUMEN
  ...
  {/* RESUMEN SEGURO Y PRIVADO */}
  <div className="resumen-gerencial"><h3>📊 Resumen de: {fechaBonita}</h3><div className="fila-resumen"><strong>Rivera:</strong> <span className="verde">Ing: ${resumenDia.rivera.entradas?.toLocaleString()}</span> | <span className="rojo">Gas: ${resumenDia.rivera.salidas?.toLocaleString()}</span></div><div className="fila-resumen"><span>Utilidad Rivera:</span><strong style={{color: resumenDia.rivera.utilidad >= 0 ? 'green' : 'red'}}>${resumenDia.rivera.utilidad?.toLocaleString()}</strong></div><hr/><div className="fila-resumen"><strong>Barcelona:</strong> <span className="verde">Ing: ${resumenDia.barcelona.entradas?.toLocaleString()}</span> | <span className="rojo">Gas: ${resumenDia.barcelona.salidas?.toLocaleString()}</span></div><div className="fila-resumen"><span>Utilidad Barce:</span><strong style={{color: resumenDia.barcelona.utilidad >= 0 ? 'green' : 'red'}}>${resumenDia.barcelona.utilidad?.toLocaleString()}</strong></div><div className="fila-resumen total" style={{flexDirection:'column', gap:'5px'}}><div style={{display:'flex', justifyContent:'space-between'}}><strong>KAMALA TOTAL:</strong></div><div style={{display:'flex', justifyContent:'space-between', fontSize:'12px'}}><span>Ing: <span className="verde">${resumenDia.total.entradas?.toLocaleString()}</span></span><span>Gas: <span className="rojo">${resumenDia.total.salidas?.toLocaleString()}</span></span></div><div style={{display:'flex', justifyContent:'space-between', marginTop:'5px', borderTop:'1px solid #ccc', paddingTop:'5px'}}><span>Utilidad Neta:</span><strong className="azul" style={{fontSize:'16px'}}>${resumenDia.total.utilidad?.toLocaleString()}</strong></div></div></div>
