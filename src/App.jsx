...
export default function App() {
  ...
  // --- DESCARGAR EXCEL (CORREGIDO TICKET PROMEDIO) ---
  const descargarReporte = async () => {
    try {
        ...
        datos.forEach(row => {
            // Aseguramos que ticket promedio sea un número normal
            let ticket = parseFloat(row.ticketPromedio) || 0;
            if (!isFinite(ticket)) ticket = 0; // Si es infinito, poner 0
            ...
            const fila = [
                ...
                (row.saldoFinalCompras ? "Ver detalle" : 0),
                row.descuadre || 0,
                Math.round(ticket) // Redondeamos para que no salgan decimales locos
            ].join(",");
            csvContent += fila + "\n";
        });
  ...
  // CORRECCIÓN CÁLCULO TICKET PROMEDIO (Evitar Infinity)
  ...
  <div className="form-card" style={{textAlign:'center', backgroundColor:'#e8f5e9'}}><h3>📊 Reportes y Estadísticas</h3><p className="nota-mini">Descarga toda la base de datos a tu PC:</p><button className="btn-guardar-cierre" style={{marginTop:'10px', background:'#2e7d32'}} onClick={descargarReporte}>📥 DESCARGAR REPORTE EXCEL</button></div>
