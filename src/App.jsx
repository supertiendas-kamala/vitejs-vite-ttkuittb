...
export default function App() {
  ...
  // 1. ESCUCHAR TODO (Ahora traemos todo y filtramos abajo)
  useEffect(() => {
  ...
  // 3. BUSCAR SALDO SUGERIDO (Solo de la misma sede)
  useEffect(() => {
      const fetchUltimoCierreSede = async () => {
          ...
          try {
              ...
              // Buscamos el último de ESTA sede
  ...
  // CORRECCIÓN 1: AHORA GUARDAMOS SEDE Y TURNO EN CADA GASTO
  const registrarEnCaja = async (desc, val, tipo) => { 
      await addDoc(collection(db, "movimientos"), { 
          proveedor: desc, monto: parseFloat(val), tipoPago: 'Efectivo', tipo: tipo, hora: new Date().toLocaleTimeString(), fecha: fecha,
          sede: sede, turno: turno // <--- ¡ESTO FALTABA!
      }); 
  };
  const agregarCompra = async () => { 
      if (!proveedor || !monto) return alert('Faltan datos'); 
      await addDoc(collection(db, "movimientos"), { 
          proveedor, monto: parseFloat(monto), tipoPago: tipoMovimiento === 'INGRESO' ? 'Efectivo' : tipoPago, tipo: tipoMovimiento, hora: new Date().toLocaleTimeString(), fecha: fecha,
          sede: sede, turno: turno // <--- ¡ESTO FALTABA!
      }); 
      setProveedor(''); setMonto(''); 
  };
  ...
  // CORRECCIÓN 2: FILTRADO ESTRICTO (POR FECHA, SEDE Y TURNO)
  // Nota: Si el registro viejo no tiene sede/turno, no saldrá.
