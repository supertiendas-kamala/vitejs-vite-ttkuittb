...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, "cierres_turnos", idCierre), (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Recargar estado de auditoría de este cierre específico si existiera (opcional)
      } 
    });
    return () => unsubscribe();
  }, [fecha, sede, turno]);
  ...
  // FILTRO ESPECIAL PARA AUDITORIA JEFA (TODO EL DÍA)
  ...
  // CALCULADORA DE BANCOS TOTALES DEL DÍA (ADMIN)
  ...
  // EXCEL DESBLOQUEADO (FUERZA BRUTA - TRAE TODO)
  const descargarReporte = async () => {
    try {
        const q = collection(db, "cierres_turnos"); // Traemos todo sin filtro de orden
  ...
  {/* CONCILIACIÓN TOTAL DEL DÍA (SUMADA) */}
  <div className="form-card"><h3>🏦 Conciliación Total del Día</h3><p className="nota-mini">Suma de todos los turnos de {fechaBonita}:</p>
  ...
  {/* AUDITORÍA DE FACTURAS (TODAS LAS DEL DÍA) */}
