...
import { collection, addDoc, onSnapshot, deleteDoc, doc, query, orderBy, limit, getDocs, where } from 'firebase/firestore';
...
export default function App() {
  ...
  // 2. RECUPERAR CIERRE GUARDADO (AHORA INCLUYE SALDO INICIAL)
  useEffect(() => {
    const cargarCierreGuardado = async () => {
      if (!sede || !turno || !fecha) return;
      ...
      // No borramos saldoInicial aun para ver si el cierre lo trae

      try {
        const q = query(collection(db, "cierres_turnos"), where("fecha", "==", fecha), where("sede", "==", sede), where("turno", "==", turno));
        ...
        if (!snapshot.empty) {
          ...
          // RECUPERAMOS EL SALDO INICIAL GUARDADO
          if (datos.saldoInicialGuardado) {
             setSaldoInicial(datos.saldoInicialGuardado);
          }

          setZSistema(datos.zSistema || ''); 
          setDevoluciones(datos.devoluciones || '');
          setEfectivoVentas(datos.ventaEfectivoSolo || '');
          setDatafono(datos.bancos?.datafono || '');
          setQrDatafono(datos.bancos?.qrDatafono || '');
          setQrBancolombiaRiv(datos.bancos?.qrRiv || '');
          setQrBancolombiaBar(datos.bancos?.qrBar || '');
          setQrBold(datos.bancos?.qrBold || '');
          setDaviKamalaRiv(datos.bancos?.daviRiv || '');
          setDaviKamalaBar(datos.bancos?.daviBar || '');
          setCantVentas(datos.numVentas || '');
          setCantDatafonos(datos.numDatafonos || '');
        }
      } catch (error) { console.error("Error recuperando cierre:", error); }
    };
    cargarCierreGuardado();
  }, [fecha, sede, turno]);
  ...
  useEffect(() => {
      const fetchUltimoCierre = async () => {
          try {
              const qCierres = query(collection(db, "cierres_turnos"), orderBy("timestamp", "desc"), limit(1));
              const querySnapshot = await getDocs(qCierres);
              if (!querySnapshot.empty) {
                  const ultimo = querySnapshot.docs[0].data();
                  const sugerido = (parseFloat(ultimo.saldoFinalCompras) || 0) + (parseFloat(ultimo.ventaEfectivoSolo) || 0);
                  setSaldoSugerido(sugerido);
              }
          } catch (e) { console.log("No hay cierres previos"); }
      };
      fetchUltimoCierre();
  }, []);
  ...
  const cerrarSesion = () => { 
    ...
    setSaldoInicial(''); // Limpiamos saldo al salir
    setZSistema(''); setDevoluciones(''); setEfectivoVentas(''); 
    setDatafono(''); setQrDatafono(''); setQrBancolombiaRiv(''); setQrBancolombiaBar(''); setQrBold(''); setDaviKamalaRiv(''); setDaviKamalaBar('');
    setCantVentas(''); setCantDatafonos(''); 
    setRSaldoPlataforma(''); setREfectivoFisico(''); setRComision(''); setValorModificarBase(''); setMostrarInputBase(false); setBaseRecargas(928000); 
  ...
  const registrarEnCaja = async (desc, val, tipo) => { await addDoc(collection(db, "movimientos"), { proveedor: desc, monto: parseFloat(val), tipoPago: 'Efectivo', tipo: tipo, hora: new Date().toLocaleTimeString(), fecha: fecha }); };
  const agregarCompra = async () => { if (!proveedor || !monto) return alert('Faltan datos'); await addDoc(collection(db, "movimientos"), { proveedor, monto: parseFloat(monto), tipoPago: tipoMovimiento === 'INGRESO' ? 'Efectivo' : tipoPago, tipo: tipoMovimiento, hora: new Date().toLocaleTimeString(), fecha: fecha }); setProveedor(''); setMonto(''); };
  ...
  const borrarDeudaRestaurante = async (id) => { if (window.confirm("¿Seguro borrar esta deuda? (Esto NO devuelve dinero)")) { await deleteDoc(doc(db, "deudas_restaurante", id)); } };
  ...
  // --- CÁLCULOS ---
  const movimientosDelDia = listaCompras.filter(m => m.fecha === fecha);
  ...
  // SALDO FINAL (Usa el saldo inicial digitado)
  ...
  const guardarCierreTurno = async () => {
    ...
    try {
        await addDoc(collection(db, "cierres_turnos"), { 
            ...
            // AQUÍ GUARDAMOS EL SALDO INICIAL
        ...
        setVista('MENU'); 
    } catch (e) { alert("Error guardando cierre: " + e.message); }
  };
  ...
  if (vista === 'COMPRAS') { return ( 
    <div className="dashboard-container"><div className="top-bar"><button className="btn-back" onClick={() => setVista('MENU')}>⬅ Volver</button><h2>Compras y Caja</h2></div>
    <div className={`saldo-card ${saldoFinalCompras < 0 ? 'bg-red' : ''}`}>
