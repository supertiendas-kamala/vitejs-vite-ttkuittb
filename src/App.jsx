...
export default function App() {
  ...
  const [cantVentas, setCantVentas] = useState(''); // Número de facturas
  const [cantDatafonos, setCantDatafonos] = useState(''); // Cantidad de vouchers
  ...
  const cerrarSesion = () => { 
    setDatosTurno(null); setVista('LOGIN'); setSede(''); setCajero(''); setTurno(''); setSaldoInicial(''); 
    ...
    setCantVentas(''); setCantDatafonos(''); // Limpiar estadísticas
  ...
  // NUEVO: GUARDAR EL CIERRE EN LA NUBE PARA ESTADISTICAS
  const guardarCierreTurno = async () => {
    ...
    try {
        await addDoc(collection(db, "cierres_turnos"), {
            fecha: fecha,
            sede: datosTurno.sede,
            cajero: datosTurno.cajero,
            turno: datosTurno.turno,
            ventaReal: totalVentaRegistrada,
            numVentas: parseFloat(cantVentas) || 0,
            numDatafonos: parseFloat(cantDatafonos) || 0,
            ticketPromedio: ticketPromedio,
            descuadre: descuadre,
            hora: new Date().toLocaleTimeString()
        });
        alert("✅ ¡Cierre guardado en la nube! Tus estadísticas se están alimentando.");
    } catch (e) {
  ...
  const saldoFinalCompras = (parseFloat(saldoInicial) || 0) - totalGastadoEfectivo + totalIngresado;
  ...
  const totalBancos = (parseFloat(datafono)||0) + (parseFloat(qrDatafono)||0) + 
                      (parseFloat(qrBancolombiaRiv)||0) + (parseFloat(qrBancolombiaBar)||0) + 
                      (parseFloat(qrBold)||0) + (parseFloat(daviKamalaRiv)||0) + (parseFloat(daviKamalaBar)||0);
