...
export default function App() {
  ...
  useEffect(() => {
    const calcularResumen = async () => {
        ...
        const calcPorSede = (nombreSede) => {
            ...
            const gastos = movs.filter(m => m.tipo === 'GASTO' && m.sede && m.sede.includes(nombreSede)).reduce((acc, el) => acc + (el.monto || 0), 0);
  ...
  // RECUPERAR CIERRE (CRUCIAL: Depende de Sede y Turno)
  useEffect(() => {
    const idCierre = generarIdCierre();
    // Si no hay ID (porque en Admin no se ha seleccionado sede/turno), limpiamos
    if (!idCierre) {
    ...
    const unsubscribe = onSnapshot(doc(db, "cierres_turnos", idCierre), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setCierreYaGuardado(false);
        if (vista === 'ADMIN') {
            // Si estamos en admin y no hay datos, limpiar visualmente los checks
            setBancosConciliados({ datafono: false, qrDatafono: false, qrRiv: false, qrBar: false, qrBold: false, daviRiv: false, daviBar: false });
            setIdsFacturasAuditadas([]);
        }
      }
    ...
    return () => unsubscribe();
  }, [fecha, sede, turno, vista]); // Se actualiza si cambian estos datos
  ...
  const cerrarSesion = () => { 
      ...
      setZSistema(''); setDevoluciones(''); setEfectivoVentas(''); setDatafono(''); setQrDatafono(''); setQrBancolombiaRiv(''); setQrBancolombiaBar(''); setQrBold(''); setDaviKamalaRiv(''); setDaviKamalaBar(''); setCantVentas(''); setCantDatafonos(''); setRSaldoPlataforma(''); setREfectivoFisico(''); setRComision(''); setValorModificarBase(''); setMostrarInputBase(false); setIdsFacturasAuditadas([]); setBancosConciliados({ datafono: false, qrDatafono: false, qrRiv: false, qrBar: false, qrBold: false, daviRiv: false, daviBar: false }); 
  };
  ...
  // FILTRO CORREGIDO PARA AUDITORÍA
  const movimientosDelDia = listaCompras.filter(m => 
      m.fecha === fecha && 
      (m.sede === sede || (!m.sede && sede)) && // Muestra si coincide sede O si es huerfano (viejos)
      (m.turno === turno || (!m.turno && turno))
  );
  ...
  const guardarCierreTurno = async () => {
    ...
    try {
        await setDoc(doc(db, "cierres_turnos", idCierre), { 
            fecha, sede, cajero, turno, zSistema, devoluciones, ventaEfectivoSolo: parseFloat(efectivoVentas) || 0,
  ...
  // CORRECCIÓN EXCEL: Sin orderBy timestamp para evitar error de índices
  const descargarReporte = async () => {
    try {
        const q = query(collection(db, "cierres_turnos")); // Traemos todo y ordenamos aquí
        const querySnapshot = await getDocs(q);
        let datos = querySnapshot.docs.map(doc => doc.data());
        if (datos.length === 0) return alert("No hay datos");
        
        // Ordenar por fecha y hora (manual)
  ...
  // ADMIN: AHORA TIENE SELECTORES PARA AUDITAR ESPECÍFICAMENTE
  ...
  // ... (COMPRAS, VENTAS, RESTAURANTE, RECARGAS y RESERVA se mantienen igual que antes) ...
  ...
  <div className="lista-compras" style={{marginTop:'20px'}}><h3>Historial de Reserva</h3>{listaReserva.length === 0 ? (<p className="empty-msg">No hay movimientos registrados.</p>) : (listaReserva.map((item) => (<div key={item.id} className="item-compra"><div className="info"><span className="hora">{item.fecha} - {item.hora}</span><strong>{item.accion === 'METER' ? 'Ingreso a Reserva' : 'Retiro de Reserva'}</strong></div><div className={`valor ${item.accion === 'METER' ? 'text-green' : 'text-red'}`}>{item.accion === 'METER' ? '+' : '-'}${item.valor?.toLocaleString()}</div></div>)))}</div></div> ); }
}