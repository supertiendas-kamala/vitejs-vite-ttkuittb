...
export default function App() {
  ...
  // 3. BUSCAR SALDO SUGERIDO (FÓRMULA GEMELOS FANTASTICOS)
  // Busca el último cierre DE LA SEDE SELECCIONADA
  useEffect(() => {
      const fetchUltimoCierreSede = async () => {
          if (!sede) return;
          try {
              // Traemos los ultimos 20 cierres y buscamos el de la sede en JS
              const qCierres = query(collection(db, "cierres_turnos"), orderBy("timestamp", "desc"), limit(20));
              const querySnapshot = await getDocs(qCierres);
              
              // Buscamos el primer cierre que coincida con la sede actual
              const ultimoSede = querySnapshot.docs.find(doc => doc.data().sede === sede);

              if (ultimoSede) {
                  const data = ultimoSede.data();
                  // FÓRMULA: Sobrante Compras + Efectivo Ventas
  ...
  }, [sede]); // Se dispara cuando cambias de sede
  ...
  if (vista === 'COMPRAS') { 
    // DIFERENCIA DE SALDO (Validación)
    ...
    {/* SECCIÓN DE VALIDACIÓN DE SALDO */}
    <div className="form-card" style={{border: '2px solid #3498db', backgroundColor: '#eaf2f8'}}>
        {saldoSugerido !== null ? (
            <div style={{marginBottom: '10px', borderBottom: '1px solid #ccc', paddingBottom: '5px'}}>
                <p style={{margin:0, fontSize:'14px', color:'#7f8c8d'}}>🤖 El sistema calcula que deberían haber:</p>
                <h3 style={{margin:0, color:'#2c3e50'}}>${saldoSugerido.toLocaleString()}</h3>
            </div>
        ) : (
            <p style={{fontStyle:'italic', fontSize:'12px', color:'#999'}}>Sin datos previos para sugerir saldo.</p>
        )}
        ...
        {saldoSugerido !== null && diferenciaSaldo !== 0 && (
            <p style={{color:'red', fontWeight:'bold', marginTop:'5px'}}>⚠️ Diferencia: ${diferenciaSaldo.toLocaleString()}</p>
        )}
        {saldoSugerido !== null && diferenciaSaldo === 0 && (
            <p style={{color:'green', fontWeight:'bold', marginTop:'5px'}}>✅ ¡Coincide Perfecto!</p>
        )}
    </div>
