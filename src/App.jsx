...
// AYUDA VISUAL PARA VER LOS MILLONES
...
export default function App() {
  ...
  // CÁLCULO INTELIGENTE (NO SUMA RESERVAS NI PRESTAMOS A GASTOS)
  useEffect(() => {
    const calcularResumen = async () => {
        ...
        // FILTRO BLINDADO: Solo gastos reales
      ...
      const fetchUltimoCierreSede = async () => {
          ...
          try {
              ...
              const ultimoSede = querySnapshot.docs.find(doc => {
                  const d = doc.data();
                  return d.sede === sede && doc.id !== idActual; // Busca el anterior distinto al actual
              });
  ...
  // --- CALCULADORA DE GASTOS REALES (Excluye Reserva y Prestamos) ---
  const gastosRealesDelDia = listaCompras.filter(m => 
      m.fecha === fecha && 
      m.tipo === 'GASTO' &&
      !m.proveedor.toLowerCase().includes("reserva") && 
      !m.proveedor.toLowerCase().includes("préstamo") &&
  ...
  // Para el Excel guardamos los gastos REALES, no los traslados
  const totalGastosRealesTurno = movimientosDelDia.filter(c => c.tipo === 'GASTO' && c.tipoPago === 'Efectivo' && !c.proveedor.toLowerCase().includes("reserva") && !c.proveedor.toLowerCase().includes("préstamo")).reduce((acc, curr) => acc + curr.monto, 0);
  ...
  const guardarCierreTurno = async () => {
    ...
    try {
        await setDoc(doc(db, "cierres_turnos", idCierre), { 
            ...
            gastosTurno: totalGastosRealesTurno, // Guardamos el gasto REAL filtrado
  ...
  // EXCEL FUERZA BRUTA (Trae todo sin filtro)
  ...
  // ... ADMIN, MENU, RECARGAS IGUALES ...
  // (Aquí va el resto del código igual que antes, pero como App.jsx ya está completo arriba, con copiar lo de arriba basta)
