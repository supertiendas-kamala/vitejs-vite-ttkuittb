...
export default function App() {
  ...
  // ADMIN VARIABLES
  const [cierresDelDia, setCierresDelDia] = useState([]); // Para guardar todos los cierres de la fecha
  ...
  // --- CEREBRO ADMIN (TRAER TODOS LOS CIERRES DEL DÍA) ---
  useEffect(() => {
    const obtenerCierresDelDia = async () => {
        // Busca TODOS los cierres de la fecha seleccionada (Sin importar sede/turno)
        ...
        // CALCULAR RESUMEN GERENCIAL
        const movsRiv = listaCompras.filter(m => m.fecha === fecha && m.sede && m.sede.includes("Rivera"));
        const movsBar = listaCompras.filter(m => m.fecha === fecha && m.sede && m.sede.includes("Barcelona"));
  ...
  }, [fecha, listaCompras, vista]); // Se actualiza con la fecha o al entrar al admin
  ...
  const cerrarSesion = () => { setDatosTurno(null); setVista('LOGIN'); setSede(''); setCajero(''); setTurno(''); setSaldoInicial(''); setZSistema(''); setDevoluciones(''); setEfectivoVentas(''); setDatafono(''); setQrDatafono(''); setQrBancolombiaRiv(''); setQrBancolombiaBar(''); setQrBold(''); setDaviKamalaRiv(''); setDaviKamalaBar(''); setCantVentas(''); setCantDatafonos(''); setRSaldoPlataforma(''); setREfectivoFisico(''); setRComision(''); setValorModificarBase(''); setMostrarInputBase(false); setBaseRecargas(928000); setIdsFacturasAuditadas([]); };
  ...
  // EXCEL SIN FILTROS RAROS (BAJA TODO)
  ...
  // CALCULADORA DE TOTALES BANCARIOS DEL DÍA (PARA ADMIN)
  ...
  // VISTA ADMIN (KAMALA) - CON TOTALES SUMADOS
  ...
  {/* CONCILIACIÓN SUMADA DEL DÍA */}
  <div className="form-card"><h3>🏦 Conciliación Total del Día</h3><p className="nota-mini">Suma de todos los turnos de hoy:</p>
  <ul className="lista-bancos-simple">
    <li>Datafono Rivera: <strong>${bancosDia.datafono.toLocaleString()}</strong></li>
    <li>QR Datáfono: <strong>${bancosDia.qrDatafono.toLocaleString()}</strong></li>
    <li>QR Bancolombia Rivera: <strong>${bancosDia.qrRiv.toLocaleString()}</strong></li>
    <li>QR Bancolombia Barce: <strong>${bancosDia.qrBar.toLocaleString()}</strong></li>
    <li>QR BOLD Unificado: <strong>${bancosDia.qrBold.toLocaleString()}</strong></li>
    <li>DaviKamala Rivera: <strong>${bancosDia.daviRiv.toLocaleString()}</strong></li>
    <li>DaviKamala Barce: <strong>${bancosDia.daviBar.toLocaleString()}</strong></li>
  </ul>
  <div className="total-conciliado">Total Bancos Día: <strong>${totalConciliadoDia.toLocaleString()}</strong></div>
  </div>
  
  <div className="form-card"><h3>📝 Auditoría de Facturas</h3><p className="nota-mini">Revisa contra el papel físico (Todas las sedes):</p>{listaCompras.filter(m => m.fecha === fecha && m.tipo === 'GASTO').length === 0 ? (<p className="empty-msg">No hay gastos registrados hoy.</p>) : (listaCompras.filter(m => m.fecha === fecha && m.tipo === 'GASTO').map((item) => (<div key={item.id} className="item-compra"><div className="info"><span className="hora">{item.hora} - {item.sede} ({item.turno})</span><strong>{item.proveedor}</strong><span className="tipo-pago">{item.tipoPago}</span></div><div className="valor text-red">-${item.monto.toLocaleString()}</div></div>)))}</div></div> ); }

  // VISTAS NORMALES (COMPRAS, VENTAS, ETC. IGUAL QUE ANTES)
  if (vista === 'COMPRAS') { 
