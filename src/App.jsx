...
export default function App() {
  ...
  const hoy = new Date().toISOString().split('T')[0]; // Fecha de HOY exacta
  ...
  // --- CEREBRO ADMIN (RESUMEN CON FILTRO DE GASTOS REALES) ---
  useEffect(() => {
    const obtenerDatosDia = async () => {
        const q = query(collection(db, "cierres_turnos"), where("fecha", "==", fecha));
        const snap = await getDocs(q);
        const encontrados = snap.docs.map(d => d.data());
        setCierresDelDia(encontrados);
        
        // Filtramos movimientos que NO sean traslados internos para el cálculo de gastos reales
        const movsReales = listaCompras.filter(m => 
            m.fecha === fecha && 
            !m.proveedor.includes("Reserva Kamala") && 
            !m.proveedor.startsWith("Préstamo Rest:")
        );

        const movsRiv = movsReales.filter(m => m.sede && m.sede.includes("Rivera"));
        const movsBar = movsReales.filter(m => m.sede && m.sede.includes("Barcelona"));
        
        const calcPorSede = (nombreSede, movimientosSede) => {
            const cierresSede = encontrados.filter(c => c.sede && c.sede.includes(nombreSede));
            const ventas = cierresSede.reduce((acc, el) => acc + (el.ventaReal || 0), 0);
            // Aquí ya solo suman los gastos reales filtrados arriba
            const gastos = movimientosSede.filter(m => m.tipo === 'GASTO').reduce((acc, el) => acc + (el.monto || 0), 0);
            return { ventas, gastos, utilidad: ventas - gastos };
        };

        const riv = calcPorSede("Rivera", movsRiv);
        const bar = calcPorSede("Barcelona", movsBar);
  ...
  // RECUPERAR CIERRE (CAJEROS)
  useEffect(() => {
    const idCierre = generarIdCierre();
    if (!idCierre) return;
    setZSistema(''); setDevoluciones(''); setEfectivoVentas(''); setDatafono(''); setQrDatafono(''); setQrBancolombiaRiv(''); setQrBancolombiaBar(''); setQrBold(''); setDaviKamalaRiv(''); setDaviKamalaBar(''); setCantVentas(''); setCantDatafonos(''); setCierreYaGuardado(false); setRSaldoPlataforma(''); setREfectivoFisico(''); setRComision(''); setIdsFacturasAuditadas([]);
  ...
  useEffect(() => {
      const fetchUltimoCierreSede = async () => {
          if (!sede) return;
          try {
              const qCierres = query(collection(db, "cierres_turnos"), orderBy("timestamp", "desc"), limit(50)); // Buscamos más atrás por si acaso
              const querySnapshot = await getDocs(qCierres);
              // Filtramos manualmente la última de la sede actual que NO sea la que estamos editando hoy (si existiera)
              const ultimoSede = querySnapshot.docs.find(doc => doc.data().sede === sede && doc.data().fecha !== fecha);

              if (ultimoSede) {
                  const data = ultimoSede.data();
                  // Sugerido = Saldo Final que quedó + Efectivo Ventas que quedó
          ...
          } catch (e) { console.log("Error trayendo sugerido", e); setSaldoSugerido(null); }
      };
  ...
  // Movimientos para vista del cajero (sin filtrar por tipo)
  const movimientosDelDia = listaCompras.filter(m => m.fecha === fecha && m.sede === sede && m.turno === turno);
  
  // FILTRO PARA AUDITORIA JEFA (TODO EL DÍA)
  ...
  // CALCULADORA BANCOS TOTALES DÍA (ADMIN)
  ...
  // GUARDAR CIERRE CON CANDADO DE FECHA 🔒
  const guardarCierreTurno = async () => {
    if (fecha !== hoy) return alert("⛔ Error: No puedes modificar cierres de fechas pasadas. Contacta al Administrador.");
  ...
  const guardarRevisionAdmin = async () => { alert("✅ Visto bueno guardado localmente (Proximamente en Nube)"); };

  // EXCEL REFORZADO (Intento agresivo de descarga)
  const descargarReporte = async () => {
    try {
        ...
        if (datos.length === 0) return alert("No se encontraron datos de cierres en la base de datos.");
  ...
  <div className="resumen-gerencial"><h3>📊 Resumen Gerencial: {fechaBonita}</h3><p className="nota-mini" style={{color:'white'}}>(Excluye préstamos y movimientos de reserva)</p><div className="fila-resumen"><strong>Rivera:</strong> <span className="verde">Ventas: ${resumenDia.rivera.ventas?.toLocaleString()}</span> | <span className="rojo">Gas: ${resumenDia.rivera.gastos?.toLocaleString()}</span></div><div className="fila-resumen"><span>Utilidad Rivera:</span><strong style={{color: resumenDia.rivera.utilidad >= 0 ? 'green' : 'red'}}>${resumenDia.rivera.utilidad?.toLocaleString()}</strong></div><hr/><div className="fila-resumen"><strong>Barcelona:</strong> <span className="verde">Ventas: ${resumenDia.barcelona.ventas?.toLocaleString()}</span> | <span className="rojo">Gas: ${resumenDia.barcelona.gastos?.toLocaleString()}</span></div><div className="fila-resumen"><span>Utilidad Barce:</span><strong style={{color: resumenDia.barcelona.utilidad >= 0 ? 'green' : 'red'}}>${resumenDia.barcelona.utilidad?.toLocaleString()}</strong></div><div className="fila-resumen total" style={{flexDirection:'column', gap:'5px'}}><div style={{display:'flex', justifyContent:'space-between'}}><strong>KAMALA TOTAL:</strong></div><div style={{display:'flex', justifyContent:'space-between', fontSize:'12px'}}><span>Ventas: <span className="verde">${resumenDia.total.ventas?.toLocaleString()}</span></span><span>Gas: <span className="rojo">${resumenDia.total.gastos?.toLocaleString()}</span></span></div><div style={{display:'flex', justifyContent:'space-between', marginTop:'5px', borderTop:'1px solid #ccc', paddingTop:'5px'}}><span>Utilidad Neta:</span><strong className="azul" style={{fontSize:'16px'}}>${resumenDia.total.utilidad?.toLocaleString()}</strong></div></div></div>
  <div className="form-card" style={{textAlign:'center', backgroundColor:'#e8f5e9'}}><h3>📊 Reportes</h3><p className="nota-mini">Descarga toda la BD:</p><button className="btn-guardar-cierre" style={{marginTop:'10px', background:'#2e7d32'}} onClick={descargarReporte}>📥 DESCARGAR EXCEL</button></div>
  
  <div className="form-card"><h3>🏦 Conciliación Total del Día (Checks Interactivos)</h3><p className="nota-mini">Suma de todos los turnos. Marca lo conciliado:</p>
  <div className="lista-checks">
    <div className="item-check" onClick={() => toggleBanco('datafono')}><div className={`checkbox-sq ${bancosConciliados.datafono ? 'checked' : ''}`}></div><span>Datafono Rivera (${bancosDia.datafono.toLocaleString()})</span></div>
    <div className="item-check" onClick={() => toggleBanco('qrDatafono')}><div className={`checkbox-sq ${bancosConciliados.qrDatafono ? 'checked' : ''}`}></div><span>QR Datáfono (${bancosDia.qrDatafono.toLocaleString()})</span></div>
    <div className="item-check" onClick={() => toggleBanco('qrRiv')}><div className={`checkbox-sq ${bancosConciliados.qrRiv ? 'checked' : ''}`}></div><span>QR Bancolombia Rivera (${bancosDia.qrRiv.toLocaleString()})</span></div>
    <div className="item-check" onClick={() => toggleBanco('qrBar')}><div className={`checkbox-sq ${bancosConciliados.qrBar ? 'checked' : ''}`}></div><span>QR Bancolombia Barcelona (${bancosDia.qrBar.toLocaleString()})</span></div>
    <div className="item-check" onClick={() => toggleBanco('qrBold')}><div className={`checkbox-sq ${bancosConciliados.qrBold ? 'checked' : ''}`}></div><span>QR BOLD Unificado (${bancosDia.qrBold.toLocaleString()})</span></div>
    <div className="item-check" onClick={() => toggleBanco('daviRiv')}><div className={`checkbox-sq ${bancosConciliados.daviRiv ? 'checked' : ''}`}></div><span>DaviKamala Rivera (${bancosDia.daviRiv.toLocaleString()})</span></div>
    <div className="item-check" onClick={() => toggleBanco('daviBar')}><div className={`checkbox-sq ${bancosConciliados.daviBar ? 'checked' : ''}`}></div><span>DaviKamala Barce (${bancosDia.daviBar.toLocaleString()})</span></div>
  </div>
  <div className="total-conciliado">Total Bancos Día: <strong>${totalConciliadoDia.toLocaleString()}</strong></div>
  </div>

  <div className="form-card"><h3>📝 Auditoría de Facturas</h3><p className="nota-mini">Revisa contra el papel físico (Todo el día):</p>{movimientosAuditoria.filter(c => c.tipo === 'GASTO').length === 0 ? (<p className="empty-msg">No hay gastos registrados hoy.</p>) : (movimientosAuditoria.filter(c => c.tipo === 'GASTO').map((item) => (<div key={item.id} className="item-compra" onClick={() => toggleFacturaAuditada(item.id)}><div className="check-area"><div className={`checkbox-sq ${idsFacturasAuditadas.includes(item.id) ? 'checked' : ''}`}></div></div><div className="info"><span className="hora">{item.hora} - {item.sede}</span><strong>{item.proveedor}</strong><span className="tipo-pago">{item.tipoPago}</span></div><div className="valor text-red">-${item.monto.toLocaleString()}</div></div>)))}</div></div> ); }
