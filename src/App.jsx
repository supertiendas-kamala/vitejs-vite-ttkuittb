...
export default function App() {
  ...
  // 1. ESCUCHAR COMPRAS (TIEMPO REAL)
  useEffect(() => {
  ...
  // 2. GESTIÓN DE DATOS DEL TURNO (CARGAR O LIMPIAR) - ¡AQUÍ ESTABA EL PROBLEMA!
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, "cierres_turnos", idCierre), (docSnap) => {
      if (docSnap.exists()) {
        ...
        // SI NO EXISTE (Turno Nuevo): ¡LIMPIEZA TOTAL! 🧹✨
        ...
        // No borramos saldoInicial porque ese es persistente para ayudar al cajero
      }
    });

    return () => unsubscribe();
  }, [fecha, sede, turno]); // Se dispara cada vez que cambias fecha, sede o turno
