...
export default function App() {
  ...
  useEffect(() => {
    const calcularResumen = async () => {
        ...
        // FILTRO BLINDADO: Solo gastos reales (Excluye Reserva y Préstamos)
  ...
  // BUSCAR SUGERIDO (Busca turno inmediatamente anterior diferente al actual)
  useEffect(() => {
      const fetchUltimoCierreSede = async () => {
          if (!sede) return;
          try {
              // Traemos los últimos cierres ordenados
              const qCierres = query(collection(db, "cierres_turnos"), orderBy("timestamp", "desc")); 
              const querySnapshot = await getDocs(qCierres);
              
              // ID del turno actual (para ignorarlo)
              const idActual = generarIdCierre();
              
              // Buscar el primer cierre de esta sede que NO sea el actual
              ...
              if (ultimoSede) {
                  ...
                  // Sugerido = Saldo Final Compras + Efectivo Ventas
  ...
  const guardarCierreTurno = async () => {
    if (!checkFecha()) return; // BLOQUEO FECHA
    if(!zSistema || !efectivoVentas) return alert("Faltan datos del cierre");
    const idCierre = generarIdCierre();
    if (!idCierre) return alert("Faltan datos sede/turno");
    
    // Calculamos el gasto real del turno (excluyendo reservas/prestamos)
    const gastoRealTurno = movimientosDelDia.filter(c => 
      c.tipo === 'GASTO' && 
      !c.proveedor.toLowerCase().includes("reserva") && 
      !c.proveedor.toLowerCase().includes("préstamo") &&
      !c.proveedor.toLowerCase().includes("prestamo")
    ...
    try {
        await setDoc(doc(db, "cierres_turnos", idCierre), { 
            ...
            gastosTurno: gastoRealTurno, // Guardamos el GASTO REAL
  ...
  // EXCEL FUERZA BRUTA (Sin dependencias de índices)
  const descargarReporte = async () => {
    try {
        console.log("Descargando...");
        // Pedimos TODA la colección sin ordenamiento
  ...
  // ... VISTAS MENU, COMPRAS, VENTAS, RECARGAS Y RESERVA IGUAL QUE ANTES ...
  // (COPIA EL BLOQUE DE ABAJO COMPLETO PORQUE TIENE CAMBIOS EN VISUALIZACIÓN)
