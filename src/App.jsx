...
export default function App() {
  ...
  // 1. CARGA INICIAL DE DATOS (LISTENERS)
  useEffect(() => {
  ...
  // 2. CÁLCULO INTELIGENTE (IGNORA RESERVA Y PRESTAMOS EN GASTOS)
  useEffect(() => {
    const calcularResumen = async () => {
        ...
        // FILTRO PODEROSO: Solo gastos reales
  ...
  // BUSCAR SUGERIDO (MEJORADO PARA NO REPETIR)
  useEffect(() => {
      const fetchUltimoCierreSede = async () => {
          if (!sede) return;
          try {
              const qCierres = query(collection(db, "cierres_turnos"), orderBy("timestamp", "desc")); // Traemos todos y filtramos en JS
              const querySnapshot = await getDocs(qCierres);
              
              // Buscar el primer cierre de esta sede que NO sea el que estamos editando ahora (por ID)
              ...
              if (ultimoSede) {
                  ...
                  // Sugerido = Lo que sobró en compras + Lo que se vendió en efectivo
  ...
  }, [sede, fecha, turno]); // Se recalcula si cambias turno también

  // --- FUNCIONES DE ACCIÓN (CON CANDADO DE FECHA) ---
  const checkFecha = () => {
      if (fecha !== hoy) {
  ...
  const modificarBase = async () => { 
      if (!checkFecha()) return; // También protegemos la reserva en fechas pasadas
      ...
      // Mover plata en caja
      if (tipoModificacion === 'SUMAR') { registrarEnCaja("Traslado a Reserva Kamala", valor, 'GASTO'); } // Aumento base = Saco de caja
      else { 
          // Disminuyo base (Navidad) = No necesariamente entra a caja, es gasto directo de reserva.
          // Depende de tu lógica. Si es pago de cena, sale del aire. Si es para caja menor, entra.
          // Por ahora solo ajustamos la base visual.
      }
  ...
  const moverReserva = async (accion, valor) => {
      ...
      // Al mover reserva, también afectamos la caja del día AUTOMÁTICAMENTE
  ...
  const guardarCierreTurno = async () => {
    if (!checkFecha()) return; // BLOQUEO DE FECHA PASADA
    ...
    try {
        await setDoc(doc(db, "cierres_turnos", idCierre), { 
            ...
            ticketPromedio, descuadre, gastosTurno: totalGastadoEfectivo, hora: new Date().toLocaleTimeString(), timestamp: Date.now()
        }, { merge: true });
  ...
  // EXCEL DESBLOQUEADO (FUERZA BRUTA)
  ...
  <div className="form-card"><h3>🏦 Conciliación Total del Día</h3><p className="nota-mini">Suma de todos los turnos de {fechaBonita}:</p><div className="lista-checks"><div className="item-check" onClick={() => toggleBanco('datafono')}><div className={`checkbox-sq ${bancosConciliados.datafono ? 'checked' : ''}`}></div><span>Datafono Rivera (${bancosDia.datafono.toLocaleString()})</span></div><div className="item-check" onClick={() => toggleBanco('qrDatafono')}><div className={`checkbox-sq ${bancosConciliados.qrDatafono ? 'checked' : ''}`}></div><span>QR Datáfono (${bancosDia.qrDatafono.toLocaleString()})</span></div><div className="item-check" onClick={() => toggleBanco('qrRiv')}><div className={`checkbox-sq ${bancosConciliados.qrRiv ? 'checked' : ''}`}></div><span>QR Bancolombia Rivera (${bancosDia.qrRiv.toLocaleString()})</span></div><div className="item-check" onClick={() => toggleBanco('qrBar')}><div className={`checkbox-sq ${bancosConciliados.qrBar ? 'checked' : ''}`}></div><span>QR Bancolombia Barcelona (${bancosDia.qrBar.toLocaleString()})</span></div><div className="item-check" onClick={() => toggleBanco('qrBold')}><div className={`checkbox-sq ${bancosConciliados.qrBold ? 'checked' : ''}`}></div><span>QR BOLD Unificado (${bancosDia.qrBold.toLocaleString()})</span></div><div className="item-check" onClick={() => toggleBanco('daviRiv')}><div className={`checkbox-sq ${bancosConciliados.daviRiv ? 'checked' : ''}`}></div><span>DaviKamala Rivera (${bancosDia.daviRiv.toLocaleString()})</span></div><div className="item-check" onClick={() => toggleBanco('daviBar')}><div className={`checkbox-sq ${bancosConciliados.daviBar ? 'checked' : ''}`}></div><span>DaviKamala Barce (${bancosDia.daviBar.toLocaleString()})</span></div></div><div className="total-conciliado">Total Bancos Día: <strong>${totalConciliadoDia.toLocaleString()}</strong></div></div>
  {/* AUDITORÍA: SELECTOR DE TURNO */}
  <div className="form-card" style={{border:'2px solid #2c3e50'}}><h3>🔍 Auditoría Detallada</h3><p className="nota-mini">Selecciona para ver facturas y checks:</p><div className="row"><select value={sede} onChange={(e) => setSede(e.target.value)}><option value="">-- Sede --</option>{sedes.map((s) => <option key={s} value={s}>{s}</option>)}</select><select value={turno} onChange={(e) => setTurno(e.target.value)}><option value="">-- Turno --</option><option value="AM">AM</option><option value="PM">PM</option></select></div></div>
