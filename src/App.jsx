...
export default function App() {
  ...
  // 2. CÁLCULO (NO SUMA RESERVA NI PRESTAMOS COMO GASTO)
  useEffect(() => {
  ...
  // BUSCAR SUGERIDO (ESTRICTO TURNO ANTERIOR)
  useEffect(() => {
  ...
  const borrarCompra = async (id) => { if (!checkFecha()) return; if (window.confirm("¿Seguro de borrar?")) { await deleteDoc(doc(db, "movimientos", id)); } };
  ...
  <div className="form-card"><h3>🏦 Conciliación Total del Día</h3><p className="nota-mini">Suma de todos los turnos de {fechaBonita}:</p><ul className="lista-bancos-simple"><li>Datafono Rivera: <strong>${bancosDia.datafono.toLocaleString()}</strong></li><li>QR Datáfono: <strong>${bancosDia.qrDatafono.toLocaleString()}</strong></li><li>QR Bancolombia Rivera: <strong>${bancosDia.qrRiv.toLocaleString()}</strong></li><li>QR Bancolombia Barce: <strong>${bancosDia.qrBar.toLocaleString()}</strong></li><li>QR BOLD Unificado: <strong>${bancosDia.qrBold.toLocaleString()}</strong></li><li>DaviKamala Rivera: <strong>${bancosDia.daviRiv.toLocaleString()}</strong></li><li>DaviKamala Barce: <strong>${bancosDia.daviBar.toLocaleString()}</strong></li></ul><div className="total-conciliado">Total Bancos Día: <strong>${totalConciliadoDia.toLocaleString()}</strong></div></div></div> ); }
